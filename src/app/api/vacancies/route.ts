...
const vacancyDraftSchema = z
  ...
  .superRefine((draft, ctx) => {
    if (draft.source === "hh" && draft.sourceUrl && !isHhVacancyUrl(draft.sourceUrl)) {
      ctx.addIssue({
        code: "custom",
        message: "URL hh должен вести на страницу вакансии (https://hh.ru/vacancy/ID), а не на поиск или служебную страницу."
      });
    }
  });
...
export async function POST(request: Request) {
  try {
    ...
    const normalizedUrl =
      draft.source === "hh" && draft.sourceUrl ? normalizeHhVacancyUrl(draft.sourceUrl) || draft.sourceUrl : draft.sourceUrl;
    const company = await findOrCreateCompany(draft.companyName);
    const vacancy = await prisma.vacancy.create({
      data: vacancyCreateData({ ...draft, sourceUrl: normalizedUrl }, company?.id ?? null, "found")
    });
